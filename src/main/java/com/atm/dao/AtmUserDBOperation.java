...
public class AtmUserDBOperation{
	...
	public boolean login(long accid,String pin) throws SQLException
	{
			
		PreparedStatement stmt=conn.prepareStatement("select * from atmuser where userId=? and userPin=? ");
	...
	public boolean changePin(long accid,String newPin) throws SQLException
	{
		PreparedStatement stmt=conn.prepareStatement("update atmuser set userPin=? where userId=? ");
	 ...
	 synchronized public void deposit(double depositAmount,long userid)
	 	{
			...
			double availableBalance=0.0;
			  ...
			  PreparedStatement stmt1=conn.prepareStatement("update atmuser set balance=? where userId=?");
	...
	public ResultSet balanceCheck(long accid)
	 	{
	 		...
	 		try
	 		{
	 		PreparedStatement pr=conn.prepareStatement("select * from atmuser where userId=?");
	 		pr.setLong(1, accid);
	 		rs=pr.executeQuery();
